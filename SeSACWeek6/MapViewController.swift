...
/*
 MapView
 - 지도와 위치 권한은 상관없다!
 - 만약 지도에 현재 위치 등을 표현하고 싶다면? 위치 권한을 등록해줘야한다.
 - 중심, 범위 지정
 - 핀(어노테이션)
 */
...
class MapViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // 37.480246, 126.898452
        // 지도 중심 설정: 애플맵 활용하여 좌표 복사
        let center = CLLocationCoordinate2D(latitude: 37.480246, longitude: 126.898452)
    ...
    func setRegionAndAnnotation(center: CLLocationCoordinate2D) {
        ...
        annotation.title = "이곳이 서울살이의 시작점이다."
...
extension MapViewController {
    ...
    func checkUserDeviceLocationServiceAuthorization() {
        ...
        if #available(iOS 14.0, *) {
            ...
            // 타입 프로퍼티 혹은 타입 메서드의 형태로 가져와서 사용한다.
        ...
        if CLLocationManager.locationServicesEnabled() {
            // 위치 서비스가 활성화 되어 있으므로, 위치 권한 요청 가능해서 위치 권한을 요청한다.
...
//            locationManager.startUpdatingLocation()
        ...
        case .authorizedWhenInUse:
            print("WHEN IN USE")
            // 사용자가 위치를 허용해둔 상태라면,
            // startUpdatingLocation을 통해 didUpdateLocations 메서드가 실행
            locationManager.startUpdatingLocation()
...
//            let center = CLLocationCoordinate2D(latitude: <#T##CLLocationDegrees#>, longitude: <#T##CLLocationDegrees#>)
            ...
            //위와 같이 따로 생성해도 되지만
            // 타입이 CLLocationCoordinate2D 이기 때문에 바로 값을 넣어줘도 상관없다.
